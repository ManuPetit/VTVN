...
if (isset($_POST['submitted'])) {//la forme est soumise
		...
		$result=mysql_query($query) or trigger_error("Query: $query\n<br />MySQL Error: " . mysql_error());
			...
			if (mysql_affected_rows() == 1) {// on a un resultat
				$body = "Etablissement numéro $etab_id a créé un événement: $nom pour le $ladate. Il est nécessaire de créer les descriptions anglaises.";				
				...
				<?php	
					...
					echo'<div id="mainMemb">';
				?>
				<p><span class="sstitre">Cr&eacute;er un &eacute;v&eacute;nement</span></p><br /><p>Votre événement a été enregistré dans la base de données.</p>
				</div>
				<?php
				print_ligne(10);
				$menu_choix =NULL;
	...
	<table border="0" cellpadding="5" width="100%">
			...
			<td>
				...
				<?php
					...
					for ($day=1; $day<=31; $day++) {
						...
						if (isset($_POST['jour'])) {//jour selectionné
							if ($_POST['jour']==$day) {
				...
				echo '<select name="mois">';
					...
					foreach ($mois as $key => $value) {
						...
						if (isset($_POST['mois'])) {//on a déja un mois
							if ($_POST['mois'] == $key) {
								echo ' selected="selected"';
							}
						} else {
				...
				echo '<select name="annee">';
					...
					for ($an=$annee1; $an<=$annee2; $an++) {
						...
						if (isset($_POST['annee'])) {//on a une année
							if ($_POST['annee'] == $an) {
								echo ' selected="selected"';
							}
						} else {
		...
		<tr>
			<td colspan="2" align="center">
				<input type="submit" name="submit" value="Enregistrer mon événement" />
			</td>
		</tr>
	</table>
	</fieldset>
